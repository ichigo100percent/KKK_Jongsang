...
//droppedStudent는 등록금을 내지 않아 해당 과정에 등록되지 않는 학생에 대한 list다.

//이 함수는 모든 과정에 현재 등록된(등록이 취소되지 않는) 학생에 대한 list를 리턴한다.
list<string> getTotalEnrollment(const vector<list<string>>& courseStudent,
								const list<string>& droppedStudents)
{
	list<string> allStudent;

	//각 과정에 대한 리스트를 모두 마스터 리스트에 합친다.
	for (auto& lst : courseStudent)
	{
		allStudent.insert(cend(allStudent), cbegin(lst), cend(lst));
	}

...
set<string> getDuplicates(const VotersMap& votersByDistrict)
{
	...
	*/
	...
	//std::pair<std::set<std::string>::iterator, bool>을 반환하며, 여기서 second는 삽입이 성공했는지 여부를 나타내는 부울 값입니다.
	//만약 이미 세트에 존재한다면 insert는 실패하고, second는 false가 됩니다
					 ...
					 const vector<string>& convictedFelons)
{
	...
	for_each
	(begin(votersByDistrict), end(votersByDistrict),
		[&toRemove](DistrictPair& district)
		{
			auto it = remove_if
			(begin(district.second), end(district.second), [&toRemove](const string& name)
				{
